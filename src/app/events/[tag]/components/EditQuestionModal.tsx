...
export function EditQuestionModal({ isOpen, onClose, question, onQuestionUpdated }: EditQuestionModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // 2. Handle Options
            // Easiest way: Delete old options and insert new ones (atomic enough for this use case if wrapped in RPC or just Sequential)
            // But we don't have an RPC for this, so let's do delete then insert.
            if (question.question_type === "select" || type === "select") {
                // Always delete existing options for this question
                ...
                // Insert new options if currently select
                if (type === "select") {
                ...
                <>
                    ...
                    >
                        <header className="px-10 pt-10 pb-6 flex justify-between items-center bg-white sticky top-0 z-10">
                            <div>
                                <h2 className="text-2xl font-black tracking-tight text-gray-900 leading-none">Edit Question</h2>
