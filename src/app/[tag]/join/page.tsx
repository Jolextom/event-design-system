...
export default function JoinPage() {
    ...
    useEffect(() => {
        ...
        const init = async () => {
            try {
                ...
                if (!attendeeData && user?.id === eventData.created_by) {
                    setIsHost(true);
    ...
    return (
        <div className={`min-h-screen bg-gradient-to-br ${meta.bg} bg-gray-950 flex flex-col items-center justify-center p-6`}
            ...
            {/* Main card */}
            <div className={`relative w-full max-w-md ring-1 ${meta.ring} bg-white/5 backdrop-blur-xl rounded-[32px] p-10 text-center border border-white/8 shadow-2xl`}>
                ...
                {isBeforeStart && (
                    <>
                        ...
                        <p className="text-gray-500 text-xs leading-relaxed">
                            You're early! This session opens 15 minutes before the start time.
                        </p>
                    </>
                )}
                ...
                {!isBeforeStart && link && joined && (
                    <>
                        ...
                        <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="w-full flex items-center justify-center gap-3 py-4 px-8 bg-white/8 hover:bg-white/12 text-white rounded-2xl font-bold tracking-tight transition-all text-sm border border-white/10"
                        >
                ...
                {/* Footer — attendee name */}
                <div className="mt-10 pt-8 border-t border-white/5 flex items-center justify-center gap-2">
                    <div className="w-7 h-7 rounded-xl bg-white/8 border border-white/10 flex items-center justify-center text-[11px] font-bold text-white/50">
